 ...
 *    				4) if you use keepalive module,you should set accept_mutex off in multiprocess environment
...
#endif
static ngx_int_t ngx_hmux_eval(ngx_http_request_t *r,ngx_hmux_loc_conf_t *hlcf);
...
static ngx_int_t ngx_hmux_input_filter(ngx_event_pipe_t *p,ngx_buf_t *buf);
static void ngx_hmux_abort_request(ngx_http_request_t *r);
static void ngx_hmux_finalize_request(ngx_http_request_t *r,ngx_int_t rc);

static ngx_int_t ngx_http_upstream_send_request_body(ngx_http_request_t *r, 
		ngx_http_upstream_t *u);
static ngx_chain_t *hmux_data_msg_send_body(ngx_http_request_t *r, size_t max_size,
		ngx_chain_t **body);
...
static int hmux_log_overflow(ngx_uint_t level,hmux_msg_t *msg, const char *context);

/**
 * protocol functions
 */
static ngx_int_t hmux_start_channel(hmux_msg_t* msg,unsigned short channel);
static ngx_int_t hmux_write_string(hmux_msg_t* msg,char code, ngx_str_t* value);
...
static ngx_chain_t* hmux_cmd_msg(ngx_hmux_ctx_t *ctx,ngx_http_request_t *r,u_char code);
static ngx_int_t hmux_msg_create_buffer(ngx_pool_t *pool, size_t size, hmux_msg_t *msg);
...
static ngx_conf_bitmask_t  ngx_hmux_ignore_headers_masks[] = {
...
ngx_hmux_create_request(ngx_http_request_t *r)
{
	...
	if (NGX_OK != rc) {
		ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
				"hmux_header_packet_buffer_size is too small:%u", hlcf->hmux_header_packet_buffer_size_conf);
	...
	if (hlcf->upstream.pass_request_body) {
		...
		cl->next = hmux_data_msg_send_body(r,hlcf->max_hmux_data_packet_size_conf, &ctx->req_body);
		last = cl;
...
ngx_http_upstream_send_request_body(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	if (ctx->state > ngx_hmux_st_request_body_data_sending) {
		ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
				"ngx_http_upstream_send_request_body: bad state(%d)", ctx->state);
	}

	cl = hmux_data_msg_send_body(r, hlcf->max_hmux_data_packet_size_conf, &ctx->req_body);
		...
		ngx_chain_t **body)
{
	...
	size_t                   base_size,added_size,redundant_size;
	...
	if ((rc = hmux_msg_create_buffer(r->pool,HMUX_DATA_SEG_SZ, msg)) != NGX_OK) {
		return NULL;
	}
		...
		if(!r->chunked&&ctx->req_body_len>0)
		{
			if(ctx->req_body_send_len>ctx->req_body_len)
			{
				...
				if(b_out->pos+redundant_size<b_out->last)
				{
					...
					ctx->req_body_send_len = ctx->req_body_send_len-redundant_size;
					actual_size = actual_size-redundant_size;
				}else
				{
					b_out->last = b_out->pos;
					ctx->req_body_send_len = ctx->req_body_send_len-added_size;
...
ngx_hmux_input_filter(ngx_event_pipe_t *p, ngx_buf_t *buf)
{
	...
	ngx_http_upstream_t *u; 
	ngx_buf_t           *b, **prev,*flush_buf;
	ngx_buf_t  			*mended_buf,*work_buf;
	u_char 		    	*pos;
	ngx_int_t 			rc;
	uint16_t 			len;
	u_char 				code;
	int 				need_read_resp_data,omit_flag;
	ngx_str_t 			str;
	int 				needMoreData;
		...
		if(0 == ctx->resp_chunk_len)
		{
			...
			switch (code) {
				...
				case HMUX_FLUSH:
					...
					{
						ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
								"overflow when receiving flush command in filter");
						needMoreData = 1;
						ctx->mend_flag=ctx->mend_flag+1;
						break;
					}
				...
				case HMUX_EXIT:
					...
					{
						ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
								"successfully for overflow when receiving data in filter,mend_flag:%d",
								ctx->mend_flag);
					}
					return NGX_OK;
				default:
...
static ngx_int_t  hmux_read_byte(hmux_msg_t* msg,u_char *rvalue)
{
...
static ngx_int_t hmux_read_len(hmux_msg_t* msg,uint16_t* rlen)
{
	...
	ngx_int_t rc=hmux_read_byte(msg,&tmp) & 0xff;
	if(NGX_OK!=rc)
	{
		return rc;
	}
...
write_env(hmux_msg_t *msg, ngx_http_request_t *r)
{
	...
	j = 0; 
	for (i = 0; (ch = uri->data[i]) && ch != '?' && (j + 3) < sizeof(buf)&&i<uri->len; i++) {
		if ('%' == ch ) {
			buf[j++] = '%'; 
			buf[j++] = '2'; 
			buf[j++] = '5'; 
	...
	transferUrl.len=strlen(buf);
	transferUrl.data=(u_char*)buf;
	/* check url validation */
	rc = check_url_valid(buf,transferUrl.len);
	if(rc != NGX_OK)
	{
		return rc;
	}
	/* writes transferred url */
	rc = hmux_write_string(msg, HMUX_URL, &transferUrl);
	if(rc != NGX_OK)
	{
		return rc;
	}
	...
	sprintf(buf, "%u", port);
	portStr.len=strlen(buf);
	portStr.data=(u_char*)buf;
	rc = hmux_write_string(msg, CSE_SERVER_PORT, &portStr);
	if(rc != NGX_OK)
	{
		return rc;
	}
	...
	/* write remote port */
	rc = hmux_write_string(msg, CSE_REMOTE_PORT, &portStr);
	if(rc != NGX_OK)
	{
		return rc;
	}
	...
	if (r->headers_in.authorization!=NULL&&r->headers_in.authorization->value.len!=0)
	{
		/* write auth type */
		rc = hmux_write_string(msg, CSE_AUTH_TYPE, &r->headers_in.authorization->value);	
		if(rc != NGX_OK)
		{
			return rc;
		}
...
write_headers(hmux_msg_t *msg, ngx_http_request_t *r)
{
	...
	for( i=0; i < part->nelts; i++)
	{
		...
		if (! strncasecmp((char*)header[i].key.data, "Content-Type",sizeof("Content-Type")))
		{
		...
		else if (! strncasecmp((char*)header[i].key.data, "Content-Length",sizeof("Content-Length")))
		{
		...
		else if (! strncasecmp((char*)header[i].key.data, "Expect",sizeof("Expect"))) {
		...
		ngx_http_request_t *r, ngx_hmux_loc_conf_t *hlcf)
{
	...
	do{
		...
		switch(code)
		{
			...
			case HMUX_HEADER:
				...
				ngx_log_error(NGX_LOG_DEBUG,r->connection->log,0,"head key and value:\"%V: %V\"",&h->key, &h->value);
				if (hh && hh->handler(r, h, hh->offset) != NGX_OK) {
					ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
							"hmux_unmarshal_response: hh->handler error: \"%V: %V\"", 
							&h->key, &h->value);
			...
			case HMUX_DATA:
				...
				{
					ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
							"receive resin response data before having sent the post data");
					ctx->long_post_flag = 1;
					rc = hmux_read_string(msg,&str);
					if(rc != NGX_OK)
					{
						ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
								"overflow when receiving data command in unmarshal");
						buf->pos = pos;
						return NGX_AGAIN;
					}
					cl = ngx_alloc_chain_link(r->pool);
					...
					dataLen = u->buffer.last-u->buffer.pos;
					u->buffer.pos = u->buffer.pos-str.len-2-1;
					ngx_memcpy(u->buffer.pos, u->buffer.pos+3+str.len,dataLen); 
					u->buffer.last = u->buffer.pos+dataLen;
			...
			case HMUX_FLUSH:
				...
				{
					ngx_log_error(NGX_LOG_INFO, r->connection->log, 0,
							"overflow when receiving flush command in unmarshal");
					return NGX_AGAIN;
				}
				ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
			...
			default:
				...
				ngx_log_debug2(NGX_LOG_DEBUG_EVENT, r->connection->log, 0,
						"accept hmux default command:%s %z in unmarshal", str.data, str.len);
				break;
		}
	...
	if(HMUX_ACK==code)
	{
		if(ctx->req_body!=NULL&&!ctx->req_body_sent_over)
		{
			...
			ngx_memcpy(u->buffer.pos, u->buffer.pos+3,dataLen); 
			u->buffer.last = u->buffer.pos+dataLen;
			rc = ngx_http_upstream_send_request_body(r, u);
			if (rc != NGX_OK) {
				return rc; 
...
{
	ngx_hmux_loc_conf_t *prev = parent;
	ngx_hmux_loc_conf_t *conf = child;
